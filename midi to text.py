...
mid = mido.MidiFile('replace with absolute file path of the midi file')

textdestpath = 'replace with absolute file path of output text file'

output_array = []
time_ac = 0  # time accumulation
length = 0

def base_wave_function(input):  # change this if you want lol
    # return np.sin(input)  # sounds boring and is too quiet
    return ((input / (np.pi / 2)) % 2) - 1
...
for msg in mid:
    ...
    if msg.type == 'note_on' or msg.type == 'note_off':
        ...
        value &= 0x00FFFFFF
        value |= (type << 31)
        value |= (msg.note << 24)  # 0bCNNNNNNNTTTTTTTTTTTTTTTTTTTTTTTT
...
with open(textdestpath, 'w') as f:
    ...
    f.write("// Written by hand in 29 months, https://github.com/thaumatichthys\n\nconst uint32_t midi_array[] = {")
